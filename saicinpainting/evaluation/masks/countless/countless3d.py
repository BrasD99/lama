...
def countless5(a, b, c, d, e):
    ...
    def p2(q, r): return q * (q == r)  # q if p == q else 0
    # q if q == r == s else 0
    def p3(q, r, s): return q * ((q == r) & (r == s))

    def lor(x, y): return x + (x == 0) * y
...
def countless8(a, b, c, d, e, f, g, h):
    ...
    def p2(q, r): return q * (q == r)
    def p3(q, r, s): return q * ((q == r) & (r == s))
    def p4(p, q, r, s): return p * ((p == q) & (q == r) & (r == s))
...
def dynamic_countless3d(data):
    ...
    results4 = (pick(subproblems3[(x, y, z)], sections[w])
...
def countless3d(data):
    ...
    final_result = reduce(
        lor, (results4, results3, results2, sections[-1])) - 1
    data -= 1
    return final_result
...
def dynamic_countless_generalized(data, factor):
    ...
    def pick(a, b): return a * (a == b)
    def lor(x, y): return x + (x == 0) * y  # logical or
...
def downsample_with_averaging(array):
    ...
    output_shape = tuple(int(math.ceil(s / f))
                         for s, f in zip(array.shape, factor))
...
def benchmark():
    ...
    for fn in methods:
        ...
        mpx = N * float(data.shape[0] * data.shape[1]
                        * data.shape[2]) / total_time / 1024.0 / 1024.0
