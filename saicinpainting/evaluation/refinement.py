...
def _pyrdown_mask(mask: torch.Tensor, downsize: tuple = None, eps: float = 1e-8, blur_mask: bool = True, round_up: bool = True):
    ...
    if blur_mask == True:
        ...
        mask = F.interpolate(mask, size=downsize,
...
def _infer(
    ...
    ekernel = torch.from_numpy(cv2.getStructuringElement(
        cv2.MORPH_ELLIPSE, (15, 15)).astype(bool)).float()
    ...
    for idi in pbar:
        ...
        for idd, forward_rear in enumerate(forward_rears):
            ...
            if idd < len(devices) - 1:
                ...
                midz1, midz2 = midz1.to(
                    devices[idd+1]), midz2.to(devices[idd+1])
        ...
        pbar.set_description("Refining scale {} using scale {} ...current loss: {:.4f}".format(
...
def _get_image_mask_pyramid(batch: dict, min_side: int, max_scales: int, px_budget: int):
    ...
    if h*w > px_budget:
        ...
        image = resize(image, (h, w), interpolation='bilinear',
                       align_corners=False)
        mask = resize(mask, (h, w), interpolation='bilinear',
                      align_corners=False)
        mask[mask > 1e-8] = 1
    breadth = min(h, w)
    n_scales = min(
...
def refine_predict(
    ...
    gpu_ids = [f'cuda:{gpuid}' for gpuid in gpu_ids.replace(
        " ", "").split(",") if gpuid.isdigit()]
    n_resnet_blocks = 0
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        mask[mask >= 1e-8] = 1.0
        mask[mask < 1e-8] = 0.0
        image, mask = move_to_device(
            image, devices[0]), move_to_device(mask, devices[0])
        ...
        image_inpainted = _infer(image, mask, forward_front, forward_rears,
                                 image_inpainted, orig_shape, devices, ids, n_iters, lr)
