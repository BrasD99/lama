...
def main(predict_config: OmegaConf):
    try:
        ...
        assert isinstance(
            model, DefaultInpaintingTrainingModule), 'Only DefaultInpaintingTrainingModule is supported'
        assert isinstance(getattr(model.generator, 'model',
                          None), torch.nn.Sequential)
        ...
        with torch.no_grad():
            for img_i in tqdm.trange(len(dataset)):
                ...
                cur_out_fname = os.path.join(predict_config.outdir, os.path.splitext(
                    mask_fname[len(predict_config.indir):])[0])
                os.makedirs(os.path.dirname(cur_out_fname), exist_ok=True)

                batch = move_to_device(
                    default_collate([dataset[img_i]]), device)
                ...
                for level_i, level in enumerate(model.generator.model):
                    ...
                    if level_i in predict_config.levels:
                        ...
                        if predict_config.slice_channels:
                            cur_feats = cur_feats[:, slice(
                                *predict_config.slice_channels)]
                        ...
                        cv2.imwrite(cur_out_fname +
                                    f'_lev{level_i:02d}_norm.png', cur_feat)
